...
本实验旨在基于 Jittor 深度学习框架，利用 Stable Diffusion v2.1 模型完成少样本下的风格迁移图像生成任务。针对给定 14 种风格（每种仅 10 张参考图）的数据集，本项目采用 DreamBooth 结合 LoRA (Low-Rank Adaptation) 的微调策略。为了解决 Baseline 模型在风格一致性与文本语义对齐上的不足，本项目提出了**高阶 LoRA 秩适配**、**先验保真损失 (Prior Preservation Loss)** 以及**基于平均风格图的 Img2Img 推理初始化策略**。

最终实验结果显示，该方案在风格相似度、文本一致性及图像质量上均取得显著提升，综合评分从 Baseline 的 0.327 提升至 **0.512078**。相关分数并未经过严格仔细的超参数微调
...
本节将按实验顺序，依次介绍我在模型训练和推理优化时探索和思考过的策略。我没有盲目堆砌 Trick，而是针对 Baseline 存在的“风格语义绑定弱”、“生成结构崩坏”以及“评分指标特性”等问题，逐一提出了针对性的解决方案。
...
- **效果** ：实验现象较为复杂且具有反直觉性。简单 **消融实验** 表明，在基础配置下，开启 Text Encoder LoRA (Rank 2, 4, 8) 相比完全冻结 (Rank 0) 反而越来越导致各项指标的下降，表现为负面影响。然而，在最终的完整策略中（结合高 Prior Weight、数据增强及高 Rank UNet），为了解决 **部分生成图像难以有效附着风格纹理** 的问题，重新开启 Text Encoder LoRA 后，观察到其似乎在维持语义结构方面提供了潜在助力，但其在复杂组合下的具体增益机制仍有待进一步深入研究。

#### 2.2.4 基于平均风格图的推理初始化【推理】

标准的 Text-to-Image 生成过程从纯高斯噪声开始去噪，这种完全随机的初始化（$\mathcal{N}(0, I)$）导致生成图像在 **色调** 和 **全局光影**上具有极大的不确定性。对于风格迁移任务，色彩分布（Histogram）是风格的重要组成部分。为了解决生成图像“色调漂移”导致 `hist_score` 低下的问题，本项目参考 [chyxx](https://github.com/Chyxx/jittor-pad-pad-eos-HighQualityStyleTransfer) 方案，提出了一种基于**平均风格先验**的推理初始化策略。
...
在解决模型对训练集“死记硬背”导致泛化能力差的问题时，我深入审查了数据处理管道，发现 Baseline 存在一个严重的逻辑缺陷：由于原始训练数据多为正方形，Baseline 直接将其 `Resize` 至 512x512，随后进行的 `RandomCrop(512)` 实际上变成了“中心裁剪”甚至“无操作”（Identity Transform）。这意味着在数千步的训练中，模型看到的始终是完全静止、构图一成不变的 10 张图，极易导致过拟合。

针对此问题，我重构了 Transform 逻辑：**先将图像 Resize 至分辨率的 1.04 倍（约 532px），再进行 512px 的 RandomCrop**。这看似微小的改动，使得裁剪窗口能够在图像上进行随机游走（Shift），凭空创造出数倍于原数据集的构图变体。此外，为了进一步强迫模型关注“纹理”而非“构图”，我还引入了随机翻转（Horizontal Flip）和旋转（Rotation）作为增强手段。
...
- **效果** ：该策略有效扩充了训练数据的多样性，从根本上解决了 Baseline 数据利用率低下的问题。虽然在最终提交中，为了追求极致的直方图打分（Histogram Score），我们策略性地关闭了会导致像素统计分布剧烈变化的几何增强（如旋转），但“Resize 1.04x + Random Crop”的机制被证明是提升小样本训练鲁棒性的关键优化。
...
#### 2.3.1 跨域语义鸿沟导致的欠风格化 (Under-stylization)
...
#### 2.3.2 深度学习框架的固有随机性 (Inherent Non-determinism)
...
- **硬件**: NVIDIA 4090 GPU (多卡并行训练/推理)。
- **关键参数** (参见源自 `self-tune.yaml`):


### 3.2 工程实现

项目首先对 Baseline 的 `train.py` 进行了全量重构，拆分了训练逻辑，使得优化和拓展更加容易。随后增设 `scripts/train_all.py` 、 `scripts/infer_all.py` 等并行脚本使得训练和推理快速且方便。最终构建了全自动化的流水线 `scripts/auto_pipeline.sh` ，利用 `multiprocessing` 实现了多 GPU 上的训练和推理过程任务分发，显著缩短了整个实验周期。
...
- 单轮时间： 3张4090， 43min34s (31m 53s + 10m 10s + 4m28s)
- 核心超参数配置（完整的请见 `configs/self-tune.yaml`）
  - 训练相关参数：
    + `model.unet_lora_rank` : 32
    + `model.text_encoder_lora_rank` : 0
    + `train.max_train_steps` : 800
    + `train.learning_rate` : 1.0e-4
    + `data.prior_loss_weight` : 1.0
    + `data.do_augmentation` : False
  - 推理相关参数（没有进行微调对比，固定如下）：
    + `infer.strength` : 0.9 
    + `infer.guidance_scale` : 7.5
    + `infer.num_inference_steps` : 40
...
下表展示了本项目最终方案（Safe Strategy）与 Baseline 的指标对比：

| **Metric Name** | **Baseline Score** | **Ours (Self-Tune)** | **提升幅度** |
| --------------------------- | ------------------ | -------------------- | ------------------------ |
| **Dino Score** (结构相似)   | 0.1130             | **0.1202** | +6.4%                    |
| **Hist Score** (颜色相似)   | 0.4388             | **0.7791** | **+77.5%** |
| **FID Score** (分布距离)    | 1.7101             | **1.6746** | **+2.1%** (Lower is better) |
| **LPIPS Score** (感知相似)  | 0.5391             | **0.4287** | +20.5% (Lower is better) |
| **CLIP-R Score** (文本对齐) | 0.8293             | **0.9307** | **+12.2%** |
| **Final Score** | 0.3274             | **0.5121** | **+56.4%** |

### 4.2 结果分析

通过对比实验数据与最终得分，可以归纳出以下核心结论：

1.  **颜色维度的极致对齐 (Hist Score +77.5%)**:
    这是本项目提升最大的单项指标（0.43 -> 0.78）。这一突破得益于**“平均图初始化 (Avg Image Init)”**与**“关闭数据增强 (No Augmentation)”**的组合策略。前者在推理阶段强行注入了风格底色，后者在训练阶段允许模型“死记硬背”训练集的像素统计分布。虽然关闭增强在理论上降低了泛化性，但在小样本风格迁移的评分体系下，这是确保颜色分数最大化的最有效手段。

2.  **语义与结构的稳健性 (CLIP-R Score +12.2%)**:
    最终方案取得了极高的 CLIP-R 得分（0.93）。这主要归功于**高权重的先验保真损失 (Prior Weight 1.0)** 以及**冻结 Text Encoder (Rank 0)** 的策略。我们选择不让 Text Encoder 参与微调，虽然牺牲了部分对 "style_00" 的语义理解深度，但最大程度地保护了预训练模型对通用物体（Prompt）的理解能力，从而确保生成的物体（如车、狗）结构完整，未出现严重的崩坏或扭曲。

3.  **FID 与感知质量的优化**:
    与 Baseline 相比，FID（1.71 -> 1.67）和 LPIPS 均有改善。这表明在 `max_train_steps=800` 和 `unet_lora_rank=32` 的设置下，模型达到了良好的收敛状态，生成的图像在清晰度和感知质量上优于基线。

4.  **策略性权衡 (Trade-off)**:
    最终 0.512 的得分验证了**“求稳策略”**的成功。我们在“艺术风格化”与“内容保真度”之间选择了后者。虽然对于部分现代物体（如 Phone, Aquarium）存在欠风格化现象，但凭借极高的颜色还原度（Hist）和文本对齐度（CLIP-R），项目成功突破了 0.5 的综合评分大关。
