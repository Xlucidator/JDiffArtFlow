...
def main():
    ...
    parser.add_argument("-c", "--config", type=str, default="self-tune.yaml", help="Path to YAML config in configs/")
    args = parser.parse_args()

    max_num = args.num_styles
    dataset_root = os.path.join(project_root, "data", "train")
    outputs_root = os.path.join(project_root, "outputs", args.exp_name)
    ...
    with jt.no_grad():
        for tempid in tqdm.tqdm(range(0, max_num)):
            ...
            for i in range(0, total_prompts, batch_size):
                ...
                batch_avg_images_of_train = [avg_image_of_train for _ in batch_items]
                neg_prompt = conf.infer.negative_prompt 
                batch_neg_prompts = [neg_prompt for _ in batch_items] if neg_prompt else None

                print(f"   Generating Batch {i//batch_size + 1}: {batch_origin_prompts}")
                ...
                results = pipe(
                    ...
                    # avg image
                    image=batch_avg_images_of_train,
                ...
                for idx, img in enumerate(results):
                    img_prompt_name = batch_origin_prompts[idx]
                    img.save(f"{save_dir}/{img_prompt_name}.png")
